...
| **編輯**（帳密登入） | 依年份季別檢視所有投稿全文、圖說、下載原檔；審閱 AI 校對結果並將定稿寄給編輯部存查 |
...
3. 登入後畫面分兩個頁籤：
   ...
   **🤖 AI 校對與審稿**（模組二）
   ...
   - 改到滿意後按「📧 確認定稿並寄給編輯部」，系統會把修訂後全文＋對照表 email 給編輯部
     （`[email].recipients`）存查，**不會直接寄給投稿者本人**——逐句列出修訂容易讓投稿者
     覺得被過度挑剔，後續要不要／怎麼跟投稿者說，由編輯自行決定
...
3. 設定好之後，**新投稿送出時就會自動在背景校對一次**，不用做任何其他設定；
   後台「🤖 AI 校對與審稿」頁籤可以審閱結果、編輯定稿、寄給編輯部存查。
...
├── mailer.py                     # Gmail SMTP：投稿通知信 + AI 校對定稿信（寄給編輯部）
